...
export default class AccouuntRepository implements IAccountRepository {
  async addAuthenticationHistory({
    userId,
    loginDate,
    ip,
    token,
    device,
  }: IAuthenticationHistory): Promise<void> {
    await AuthenticationHistoryContext.updateMany(
      { userId: userId, logoutDate: undefined },
      { logoutDate: new Date() },
    );
  ...
  async updateLogout(token: string): Promise<void> {
    await AuthenticationHistoryContext.findOneAndUpdate(
      { token: token },
      { logoutDate: new Date() },
    );
  }
